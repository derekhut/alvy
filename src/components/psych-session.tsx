...
export default function PsychSession() {
  ...
  const morphemes = useMemo(
    () => selectNextMorphemes(initialData, allConcepts, dailyGoal),
    [initialData, allConcepts, dailyGoal],
  );

  const getNextBatch = useCallback((data: import("../lib/types.js").UserData) => {
    return selectNextMorphemes(data, allConcepts, data.settings?.dailyGoal ?? data.dailyGoal);
  }, [allConcepts]);
